...
:::{.figure-placeholder}
![The Robotic Fox entering the reinforcement learning jungle, where each action is shaped by feedback from the environment.](images/chapter5/fox_jungle.png){fig-align="center" width="80%"}
:::

![The Fox's Toolkit: A visual map of Reinforcement Learning algorithm families—Value-Based (Q-Learning, DQN), Policy-Based (REINFORCE, Policy Gradient), Actor-Critic (PPO, SAC), Model-Based (MuZero, Dreamer), and Modern RL (RLHF, Offline RL).](images/chapter5/rl_algorithm_ecosystem_bordered.png){#fig-rl-ecosystem}

::: {.callout-note}
## Jargon Buster: The RL Toolkit Stack

Just as the Tiger uses **scikit-learn** and the Owl uses **PyTorch**, the Fox has its own toolkit. Here are the key Python libraries for Reinforcement Learning:

| Library | Purpose | When to Use |
|:---|:---|:---|
| **Gymnasium** | Environments (simulations) | Create/use standard RL environments |
| **Stable Baselines3** | Ready-made algorithms | Quick prototyping with PPO, SAC, DQN |
| **Ray RLlib** | Distributed training | Large-scale, multi-agent RL |
| **TorchRL** | PyTorch-native RL | Custom architectures, research |
| **CleanRL** | Single-file implementations | Learning RL, education |

**The Standard Stack (2025):**
```
pip install gymnasium stable-baselines3
```

**Why Gymnasium + Stable Baselines3?**
- **Gymnasium** (successor to OpenAI Gym) provides standard environments: CartPole, LunarLander, Atari games, robotics tasks
- **Stable Baselines3** provides production-ready algorithms: PPO, SAC, DQN, A2C
- Together, they let you train an RL agent in ~10 lines of code
:::

**Technical Spotlight: Training a Fox with PPO (Stable Baselines3)**

```python
# Python: Train an RL agent in 10 lines
import gymnasium as gym
from stable_baselines3 import PPO

# 1. Create the environment (the Jungle)
env = gym.make("LunarLander-v3")

# 2. Create the agent (the Fox) with PPO algorithm
model = PPO("MlpPolicy", env, verbose=1)

# 3. Train the Fox (10,000 timesteps)
model.learn(total_timesteps=10_000)

# 4. Watch the trained Fox in action
obs, info = env.reset()
for _ in range(1000):
...
model.save("fox_lunar_lander")
print("🦊 Fox trained and saved!")
```

**Key Insight:** This is the **same pattern** as scikit-learn: `model.learn()` (like `.fit()`), `model.predict()`, and `model.save()`. The Fox learns through millions of interactions, but the API is just as simple.
...
::: {.callout-note}
## Key Concept: The RL Feedback Loop

**Agent** observes **State** → takes **Action** → receives **Reward** → updates its **Policy**

This cycle repeats thousands or millions of times until the agent learns an effective strategy.
:::

```{mermaid}
%%| label: fig-rl-loop
%%| fig-cap: "The RL Feedback Loop: Agent observes state, takes action, receives reward, updates policy."
flowchart LR
    A["🦊 Agent<br/>(Fox)"] -->|observes| S["📍 State<br/>(Environment)"]
    A -->|takes| Act["⚡ Action"]
    Act -->|affects| E["🌲 Environment<br/>(Jungle)"]
    E -->|returns| R["🎯 Reward<br/>(+10 / -15)"]
    R -->|updates| P["🧠 Policy<br/>(Strategy)"]
    P -->|guides| A
    
    style A fill:#ff9800,stroke:#e65100,color:#fff
    style S fill:#2196f3,stroke:#1565c0,color:#fff
    style Act fill:#4caf50,stroke:#2e7d32,color:#fff
    style E fill:#9c27b0,stroke:#6a1b9a,color:#fff
    style R fill:#f44336,stroke:#c62828,color:#fff
    style P fill:#00bcd4,stroke:#00838f,color:#fff
```
...
```{mermaid}
%%| label: fig-rl-family-tree
%%| fig-cap: "RL Algorithm Family Tree: The major branches of reinforcement learning methods."
flowchart TD
    ...
    style RL fill:#ff5722,stroke:#bf360c,color:#fff
    style VB fill:#2196f3,stroke:#1565c0,color:#fff
    style PB fill:#4caf50,stroke:#2e7d32,color:#fff
    style AC fill:#9c27b0,stroke:#6a1b9a,color:#fff
    style MB fill:#00bcd4,stroke:#00838f,color:#fff
    style PPO fill:#ffc107,stroke:#ff8f00,color:#000
```
...
:::{.figure-placeholder}
![Observation step: the Fox reads the current state of the jungle before choosing an action.](images/chapter5/fox_observer_state.png){fig-align="center" width="80%"}
:::
...
:::{.figure-placeholder}
![Decision step: the Fox chooses a path based on current knowledge and possible reward.](images/chapter5/FoxAndLPathAward.png){fig-align="center" width="80%"}
:::
...
:::{.figure-placeholder}
![Reward step: the Fox receives positive feedback after a successful action.](images/chapter5/fox_reward_catch_rabbit.png){fig-align="center" width="80%"}
:::

---

:::{.figure-placeholder}
![Penalty step: the Fox receives negative feedback when an action leads to a poor outcome.](images/chapter5/fox_penalty.png){fig-align="center" width="80%"}
:::
...
:::{.figure-placeholder}
![Exploration versus exploitation: the Fox pauses at a fork, balancing the safe known path against a risky unknown route.](images/chapter5/fox_at_forked_jungle_path.png){fig-align="center" width="80%"}
:::
---
...
![Figure 5.6 — Credit Assignment Echoes. The Fox receives its reward at the end of the trail, but golden light flows backward through earlier choices. RL must decide which past actions deserve credit for the outcome, not just the final step.](images/chapter5/credit_assignment_echoes.png){fig-align="center" width="80%"}
...
![Figure 5.7 — The DQN Memory Den. The Fox brings remembered journeys to the Owl, who distills them through a neural canopy. A Deep Q-Network replaces a fixed ledger with a model that can replay experience and generalize to new states.](images/chapter5/dqn_memory_den.png){fig-align="center" width="80%"}
...
%%| fig-cap: "Actor-Critic Architecture: The Actor proposes actions while the Critic evaluates them."
flowchart LR
    State["📍 State"] --> Actor["🎭 Actor<br/>(Proposes Actions)"]
    State --> Critic["📊 Critic<br/>(Evaluates Value)"]
    Actor -->|action| Env["🌲 Environment"]
    Env -->|reward| Critic
    Critic -->|feedback| Actor
    
    style Actor fill:#4caf50,stroke:#2e7d32,color:#fff
    style Critic fill:#2196f3,stroke:#1565c0,color:#fff
    style State fill:#ff9800,stroke:#e65100,color:#fff
    style Env fill:#9c27b0,stroke:#6a1b9a,color:#fff
```
...
![Figure 5.8 — Simulating Futures Before Acting. In model-based RL, the Fox imagines multiple possible jungles before choosing a path. It learns an internal model of how actions change future states and rewards.](images/chapter5/model_based_future_simulation.png){fig-align="center" width="80%"}
...
![Figure 5.9 — Reward Hacking: The Deceptive Shortcut. A poorly designed reward can lure the Fox toward easy proxy points instead of the true goal. Good reward design makes the safe, intended path more attractive than the loophole.](images/chapter5/reward_hacking_shortcut.png){fig-align="center" width="80%"}
...
::: {.callout-note}
## Quick Reference: RL Algorithm Selection

| Algorithm | Type | Best For | Complexity |
|:---|:---|:---|:---|
| **Q-Learning** | Value-based | Discrete actions, simple envs | Low |
| **DQN** | Value-based + NN | High-dimensional states | Medium |
| **Policy Gradient** | Policy-based | Continuous actions | Medium |
| **PPO** | Actor-Critic | General purpose, stable | Medium-High |
| **SAC** | Actor-Critic | Robotics, sample efficiency | High |
