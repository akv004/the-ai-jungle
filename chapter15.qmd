...
::: {.callout-warning}
## Quantum AI: Frontier Technology

Quantum computing for AI is still largely experimental. Current applications are limited, but the potential for exponential speedups in optimization and simulation makes this a critical area to watch.
:::
...
```{mermaid}
%%| fig-cap: "Figure 15.3 — Hybrid quantum-classical learning loop"
flowchart LR
  Problem["Classical problem<br/>data and objective"] --> Encode["Feature encoding<br/>map into qubits"]
  Encode --> Quantum["Quantum circuit<br/>explore candidate states"]
  Quantum --> Measure["Measurement<br/>sample outcomes"]
  Measure --> Score["Classical score<br/>loss or reward"]
  Score --> Optimizer["Classical optimizer<br/>update parameters"]
  Optimizer --> Quantum
  Score -->|Good enough| Answer["Candidate solution<br/>validated classically"]
```
