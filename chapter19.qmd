...
![The roots beneath the AI Jungle, symbolizing CPUs, GPUs, accelerators, memory, and cloud infrastructure supporting visible AI capabilities above the surface.](images/chapter19_images/RootsOfNatureAndTechnology.png){width=100% height=auto}
...
```{mermaid}
%%| fig-cap: "Figure 19.2 — Hardware selection trail for AI workloads"
flowchart TD
  Start["Start with workload"] --> Size{"Large model<br/>or high throughput"}
  Size -->|No| Cpu["CPU path<br/>prototype or small inference"]
  Size -->|Yes| Latency{"Strict latency<br/>or edge privacy"}
  Latency -->|Yes| Edge["Edge path<br/>NPU FPGA or quantized model"]
  Latency -->|No| Train{"Training<br/>or batch inference"}
  Train -->|Training| Cluster["Accelerator cluster<br/>GPU TPU or cloud pod"]
  Train -->|Inference| Serving["Serving stack<br/>CPU plus GPU autoscaling"]
  Cluster --> Cost["Check cost<br/>memory and energy budget"]
  Serving --> Cost
  Edge --> Cost
  Cpu --> Cost
```
