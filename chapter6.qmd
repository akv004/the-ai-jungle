...
![Figure 6.6 — The Owl Watching the Tides. The river has crept past three earlier watermark lines on the bank. No single day looked like a flood, but each small overflow has been remembered and added to the last.](images/chapter6_images/owl_watching_tides.png){fig-align="center" width="80%"}
...
![Figure 6.7 — The Owl Tuning Its Alarm Bells. The steady elephant lives inside a small tight halo of alarm; the erratic monkey inside a much wider one. The Owl scales each bell to the creature's natural volatility, not to a single fixed threshold.](images/chapter6_images/owl_dynamic_zscore.png){fig-align="center" width="80%"}
...
::: {.callout-tip}
## Worked Example: Two Thresholds Under Different Skies

A network-monitoring system watches hourly outbound traffic from two user accounts. The base anomaly threshold is the same for both: alert when traffic is more than `3.0` standard deviations above normal. The dynamic rule is:

`adjusted_threshold = base × (1 + CV)`

For User A, the last 30 days average `100 MB/hour`, with a standard deviation of `8 MB/hour`.

`CV = 8 / 100 = 0.08`

That places User A in the `STABLE` tier. The adjusted threshold is:

`3.0 × (1 + 0.08) = 3.24`

For User B, the last 30 days average `100 MB/hour` too, but the standard deviation is `65 MB/hour`.

`CV = 65 / 100 = 0.65`

That places User B in the `VOLATILE` tier. The adjusted threshold is:

`3.0 × (1 + 0.65) = 4.95`

Now a traffic spike with z-score `4.2` triggers for User A, because `4.2 > 3.24`, but not for User B, because `4.2 < 4.95`. The robotic owl does not call every rustle danger; it listens to how restless that part of the canopy usually is.

**Takeaway:** Dynamic z-scores make the alarm stricter for stable behavior and more tolerant for naturally variable behavior.
:::
...
![Figure 6.8 — The Lone Wolf at the Edge of the Glade. While most creatures gather densely around the glowing watering hole, a solitary wolf stands at the sparse edge — locally unusual, even if not globally distant.](images/chapter6_images/wolf_glade.png){fig-align="center" width="80%"}
