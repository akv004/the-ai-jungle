...
![The Artistic Bird, a generative AI metaphor for systems that create new images, text, music, and designs from learned patterns.](images/chapter18_images/chapter18Peacock.png){width=100% height=auto}
...
```{mermaid}
%%| fig-cap: "Figure 18.3 — Diffusion generation from prompt to finished image"
flowchart LR
  Prompt["Text prompt<br/>creative intent"] --> Encoder["Text encoder<br/>semantic embedding"]
  Noise["Random noise<br/>initial canvas"] --> Denoise["Denoising model<br/>guided cleanup loop"]
  Encoder --> Denoise
  Denoise --> Preview["Emerging image<br/>forms and colors"]
  Preview --> Detail["Refinement<br/>texture lighting details"]
  Detail --> Output["Final image<br/>prompt aligned result"]
  Detail -->|More steps| Denoise
```
