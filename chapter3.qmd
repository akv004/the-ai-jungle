...
| Desired Output | Good First Method | Why It Helps | Watch-outs |
|---|---|---|---|
| A numeric prediction | Linear Regression | Fast, explainable, and easy to debug | Check whether errors curve or widen; that may mean the relationship is not linear |
| A yes/no decision | Logistic Regression | Produces probabilities, not just labels | A 0.51 probability is very different from 0.99; choose thresholds carefully |
| A clear set of rules | Decision Tree | Easy to explain as if-then branches | Deep trees memorize training data easily |
| Strong tabular accuracy | Random Forest or Gradient Boosting | Captures interactions between features | Use cross-validation; avoid leaking future information into training |
| Customer or behavior groups | K-Means | Simple way to discover common patterns | Standardize features first; large-scale features can dominate distance |
| Strange or rare cases | Isolation Forest | Finds points that are easier to separate from the crowd | Rare does not always mean bad; verify with domain knowledge |
| A simpler feature space | PCA | Compresses many numeric columns into fewer components | Components can be harder to explain than original columns |
