...
|---|---|---|
| Goal | Learn mapping from inputs → labels | Discover structure without labels |
| Example Tasks | Regression, classification | Clustering, anomaly detection, dimensionality reduction |
| Tiger Analogy | Flashcards with answers: *gazelle / rock* | Grouping creatures by similarity: size, movement, sounds |

**Examples**
- **Regression** (Supervised): Predict housing price from location, size, age.
- **Classification** (Supervised): Email spam vs. not spam.
- **Clustering** (Unsupervised): Group shoppers with similar behavior.
- **Anomaly Detection** (Unsupervised): Flag unusual sensor readings.

Supervised learning relies on labeled data, which guides the tiger’s hunting instincts—each label acts as a clue to distinguish prey from non-prey. Unsupervised learning, on the other hand, allows the tiger to roam freely, discovering hidden patterns and structures in its environment without explicit guidance. This exploratory behavior is crucial when labels are scarce or unavailable.

### AI-generated image prompt
```text
PROMPT: Two robotic tigers in a dense jungle; one reads a glowing manual with labels (supervised learning), the other explores mysterious, uncharted paths discovering hidden patterns (unsupervised learning). Cinematic 16:9 lighting, vibrant colors, ultra-detailed.
...
|---|---|---|---|
| Numeric prediction (regression) | Linear Regression, Ridge/Lasso | Interpretable coefficients, fast | Scale features; check residuals |
| Binary/multi-class classification | Logistic Regression, Decision Tree | Fast baseline, clear thresholds | Calibrate probabilities; class imbalance |
| Tabular data with interactions | Random Forest, Gradient Boosting (XGBoost/LightGBM) | Strong accuracy with minimal feature engineering | Tune depth/learning rate; avoid leakage |
| Unlabeled exploration | K-Means, PCA | Discover structure, reduce noise | Choose `k` sensibly; standardize first |
| Visualization of structure | t‑SNE, UMAP | Reveal clusters/manifolds | Not for supervised training; sensitive to params |

**Rule of Thumb:** start simple, measure honestly, then graduate to ensembles. Let the tiger learn to walk before it sprints.

### AI-generated image prompt
```text
PROMPT: A stylized jungle map as a decision guide—paths labeled Regression, Classification, Clustering, Dimensionality Reduction—with signposts and glowing waypoints. Cinematic 16:9, ultra-detailed.
NEGATIVE: no text, no watermark, lowres, blur
SIZE: 1664x928
```
...
### AI-generated image prompt
```text
PROMPT: The tiger carefully aligning sensor dials (feature scales) before a hunt; cables route into a single clean harness (pipeline). Futuristic lab-in-jungle, cinematic 16:9.
NEGATIVE: no text, no watermark, lowres, blur
SIZE: 1664x928
```
...
### AI-generated image prompt
```text
PROMPT: The robotic tiger atop a ridge at sunset, with labeled constellations of “Data, Models, Metrics, Ethics, Reproducibility” glowing across the sky. Cinematic 16:9, inspiring, ultra-detailed.
NEGATIVE: no text, no watermark, lowres, blur
SIZE: 1664x928
```
