...
The Tiger should not choose an algorithm because it sounds impressive. It should first ask: **What real-world job am I trying to do, and what kind of answer do I need?** Use the table below as a practical starting guide.

::: {.wide-table .field-guide-table}
| Problem Type | Typical Baselines | When to Prefer / Use Cases | Example Input → Output | Watch-outs |
|---|---|---|---|---|
| **Numeric prediction (regression)** | Linear Regression, Ridge, Lasso | Use when the answer is a number. Common uses: house price, delivery time, electricity demand, next-month sales. | `size=1800 sq ft`, `3 bedrooms`, `ZIP=78701` → `$345,000` predicted price | Check whether errors are large for certain groups. Nonlinear patterns may need Random Forest or Gradient Boosting. |
| **Binary classification** | Logistic Regression, Decision Tree | Use when there are two possible classes and probabilities matter. Common uses: spam detection, churn prediction, loan approval, disease risk flag. | `sender=unknown`, `links=5`, `urgent words=12` → `spam`, `92% probability` | Watch class imbalance. A model can look accurate by always predicting the majority class. |
| **Multi-class classification** | Logistic Regression, Decision Tree, Random Forest | Use when each record belongs to one of several known categories. Common uses: support routing, product category detection, plant/species recognition. | `ticket text="charged twice"` → `billing issue` | Check confusion between similar classes. Weak categories usually need more labeled examples. |
| **Tabular data with feature interactions** | Random Forest, Gradient Boosting, XGBoost, LightGBM | Use when combinations of features matter. Common uses: fraud scoring, insurance risk, demand forecasting, lead scoring. | `new device`, `large purchase`, `foreign location` → `high fraud risk` | Avoid data leakage. Tune depth and learning rate. Explainability may be harder than with linear models. |
| **Unlabeled grouping (clustering)** | K-Means, DBSCAN, Hierarchical Clustering | Use when you do not have labels and want to discover natural groups. Common uses: customer segmentation, document grouping, behavior discovery. | `purchase frequency`, `order value`, `favorite categories` → `loyal buyer segment` | Standardize numeric features. Clusters need human interpretation; the algorithm will not name the groups for you. |
| **Anomaly detection** | Isolation Forest, Local Outlier Factor, One-Class SVM | Use when rare unusual events matter more than ordinary cases. Common uses: machine failure, suspicious login, unusual spending, sensor faults. | `temperature spike`, `high vibration`, `recent maintenance` → `suspicious reading` | Rare does not always mean bad. Alerts must be reviewed with domain knowledge. |
| **Dimensionality reduction** | PCA, t-SNE, UMAP | Use when there are too many features to inspect directly. Common uses: visualizing embeddings, compressing survey data, denoising sensor data. | `768 embedding values` → `2D map of similar documents` | t-SNE/UMAP are mainly for exploration. PCA is easier to explain but may hide original feature meaning. |
:::

#### Step 1: Check Whether You Have Labels

The first fork in the trail is simple:

- **If you have labeled examples**, use supervised learning. Example: past house records include actual sale prices, or past emails are labeled `spam` / `not spam`.
- **If you do not have labels**, use unsupervised learning. Example: you have customer purchase histories but no predefined customer types.
- **If labels are rare or expensive**, start with clustering or anomaly detection to explore the data, then label a smaller, more useful sample.

In the Tiger analogy, labels are the elder's field notes: "this track led to prey," "that shadow was only a rock." Without those notes, the Tiger can still explore, but it must discover structure on its own.

#### Step 2: Match the Output to the Method

Once you know whether labels exist, match the **shape of the answer** to the method:

- **A number:** start with Linear Regression or Ridge, then try tree ensembles if errors show nonlinear patterns.
- **A yes/no or category label:** start with Logistic Regression or a Decision Tree, then try Random Forest or Gradient Boosting when feature interactions matter.
- **A natural group:** start with K-Means after scaling numeric features.
- **A strange or rare case:** start with Isolation Forest, then ask a domain expert whether the flagged cases are actually meaningful.
- **A simpler view of many columns:** start with PCA for compression or t-SNE/UMAP for exploration.

#### Practical Examples

**Example 1: Predicting house prices**

- **Question:** "What should this house sell for?"
- **Data:** square footage, bedrooms, age, neighborhood, past sale price.
- **Start with:** Linear Regression or Ridge Regression.
- **Upgrade to:** Random Forest Regressor or Gradient Boosting if the relationship is nonlinear.
- **Metric:** Mean Absolute Error (MAE), because "off by $18,000" is easier to understand than an abstract score.

**Tiger analogy:** The Tiger estimates how far the herd has moved by reading the slope of the land, the freshness of tracks, and the time since sunrise. It is predicting a number, so it needs regression.

**Example 2: Filtering spam emails**

- **Question:** "Is this email spam or legitimate?"
- **Data:** words in the email, sender reputation, links, previous spam labels.
- **Start with:** Logistic Regression.
- **Upgrade to:** Random Forest or Gradient Boosting when word combinations and sender patterns become more complex.
- **Metric:** Precision and recall. False positives can hide real messages; false negatives let spam through.

**Tiger analogy:** The Tiger asks, "Is this rustle prey or just wind?" The answer is a category, so it needs classification.

**Example 3: Grouping customers**

- **Question:** "What kinds of customers do we have?"
- **Data:** purchase frequency, average order size, product categories, days since last purchase.
- **Start with:** K-Means after scaling the features.
- **Upgrade to:** DBSCAN if groups have irregular shapes or if you expect outliers.
- **Metric:** Silhouette score can help, but business usefulness matters more than a perfect statistic.

**Tiger analogy:** At a watering hole, the Tiger notices that some animals move in herds, some arrive alone, and some only appear at night. No one gives the Tiger labels; it discovers natural groups.

**Example 4: Detecting suspicious machine behavior**

- **Question:** "Is this sensor reading normal?"
- **Data:** temperature, vibration, pressure, error logs, time since maintenance.
- **Start with:** Isolation Forest or Local Outlier Factor.
- **Upgrade to:** Supervised classification later if engineers label enough examples of real failures.
- **Metric:** Review rate and confirmed incident rate. An anomaly detector is useful only if humans can act on its alerts.

**Tiger analogy:** Most forest sounds blend into the background. A sudden metallic screech is rare, separated from the normal rhythm, and worth investigating.

#### A Simple Starting Strategy

1. **Build the simplest reasonable baseline.** Use Linear Regression for numbers, Logistic Regression for categories, and K-Means for unlabeled groups.
2. **Measure against a holdout set.** The Tiger must prove it can hunt in new territory, not only where it trained.
3. **Inspect mistakes.** Look at wrong predictions one by one. They often reveal bad labels, missing features, or data leakage.
4. **Increase complexity only when needed.** Try Random Forests or Gradient Boosting after simple models expose the baseline.
5. **Prefer explainability when stakes are high.** In credit, healthcare, hiring, safety, or security, a slightly less accurate but explainable model may be the better tool.
...
```{mermaid}
%%| fig-cap: "Linear regression: from measured clues to one numeric prediction"
flowchart LR
  A["Measured clues<br/>size age distance"] --> B["Fit best line<br/>minimize error"]
  B --> C["Prediction<br/>one numeric value"]
  C --> D["Residual check<br/>how far off"]
  D -->|Pattern remains| E["Try richer model<br/>or add features"]
```
...
```{mermaid}
%%| fig-cap: "Logistic regression: convert evidence into a probability and decision"
flowchart LR
  A["Input features<br/>words links sender"] --> B["Weighted score<br/>evidence total"]
  B --> C["Sigmoid curve<br/>probability 0 to 1"]
  C --> D{"Threshold<br/>for action"}
  D -->|Above threshold| E["Class 1<br/>spam or high risk"]
  D -->|Below threshold| F["Class 0<br/>not spam or low risk"]
```
...
```{mermaid}
%%| fig-cap: "Decision tree: a trail of if-then questions leading to a prediction"
flowchart TD
  A["Start<br/>new example"] --> B{"Speed high"}
  B -->|Yes| C{"Size small"}
  B -->|No| D{"Near water"}
  C -->|Yes| E["Leaf<br/>likely prey"]
  C -->|No| F["Leaf<br/>ignore"]
  D -->|Yes| G["Leaf<br/>investigate"]
  D -->|No| H["Leaf<br/>low priority"]
```
...
Before moving on, place these tree methods on one trail:

- A **Decision Tree** is one rule-based path through the jungle.
- A **Random Forest** grows many trees independently and lets them vote.
- **Gradient Boosting** grows trees one after another, where each new tree focuses on earlier mistakes.
- **XGBoost** made gradient boosting robust, regularized, and widely practical.
- **LightGBM** keeps the same boosting idea but makes it faster and lighter for very large tabular datasets.

So LightGBM is not a separate branch of machine learning. It is a modern, high-speed member of the gradient boosting family.

### LightGBM: A Fast Gradient Boosting Successor
...
![Figure 3.2 — K-Means groups similar creatures around learned centers. Each cluster forms around a centroid, while distant points may deserve extra review.](images/chapter3_images/kmeans_behavioral_census.png){#fig-kmeans-clusters fig-align="center" width="80%"}
...
```{mermaid}
%%| fig-cap: "PCA: compress many correlated signals into fewer useful directions"
flowchart LR
  A["Many features<br/>height speed sound color"] --> B["Standardize<br/>same scale"]
  B --> C["Find directions<br/>largest variance"]
  C --> D["Keep top components<br/>PC1 PC2 PC3"]
  D --> E["Use compressed view<br/>plot denoise model input"]
```
...
![The Robotic Tiger at sunset, carrying the chapter's foundational machine learning lessons into the next stage of the journey.](images/chapter3_images/tiger_sunset.png){width=100% height=auto}
